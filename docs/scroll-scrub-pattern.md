...
> **출처**: `portfolio` repo `docs/scroll-scrub-pattern.md`
> **가져온 날짜**: 2026-05-17
> **상태**: 레퍼런스 보관용. 본 repo(`parkseik/main`)에는 아직 구현 안 됨.
>
> 본 repo 포팅 시 주의:
> - 아래 "참조 구현" 섹션의 파일 경로는 portfolio repo 기준. 본 repo에서 구현 시 경로 매핑 필요
> - 클래스명은 본 repo 컨벤션(단일 대시 `xxx-xxx-xxx`, BEM 금지)을 따를 것 — `CLAUDE.md` 참조
> - 미디어 URL은 본 repo 표준인 Cloudflare R2 패턴으로 치환. 아래 Cloudinary 예시는 portfolio repo 컨텍스트일 뿐

## 참조 구현 (portfolio repo 기준)
...
> 클래스명 예시는 portfolio repo 기준. 본 repo 포팅 시 단일 대시 컨벤션을 유지하므로 그대로 사용 가능 (BEM 변환 불필요).
...
CldVideoPlayer 같은 추상화 대신 네이티브 `<video>` 사용 (currentTime 직접 제어 필요).

**본 repo (parkseik/main)** — Cloudflare R2 공개 버킷 사용:

```
https://<R2_PUBLIC_BASE>/<path>.mp4
```

(R2 베이스 URL은 본 repo의 v0.3.6 changelog 및 환경 설정 참조)

**portfolio repo 원본** — Cloudinary 사용 (참고용):
